...
static const uint8_t advertised_service_uuid[16U] = { 0x62,
                                                      0x9e,
                                                      0x3c,
                                                      0xd1,
                                                      0x12,
...
static uint8_t receive_buffer[12U];
static uint32_t sensor_data[3U];
...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_connection_closed_id:
      app_log_info("Connection close, scan for other devices\r\n");
      app_connection = 0xFF;
      gatt_state = SCANNING;
      sl_bt_scanner_start(gap_1m_phy,
                          scanner_discover_observation);
    ...
    case sl_bt_evt_scanner_scan_report_id:
      /* Find server by name */
      if (findDeviceByName(&(evt->data.evt_scanner_scan_report),"FET Piltover")) {
    ...
    case sl_bt_evt_gatt_characteristic_value_id:
      if ((evt->data.evt_gatt_characteristic_value.characteristic == characteristic_handle) &&
          (evt ->data.evt_gatt_characteristic_value.att_opcode == sl_bt_gatt_handle_value_notification)) {
        for(int i=0; i< 12; i++) {
            receive_buffer[i] = evt->data.evt_gatt_characteristic_value.value.data[i];
        }
        sensor_data[0] = (receive_buffer[2] << 8) | receive_buffer[3];
        sensor_data[1] = (receive_buffer[6] << 8) | receive_buffer[7];
        sensor_data[2] = (receive_buffer[10] << 8) | receive_buffer[11];
        sl_app_log("HR %d\r\n",sensor_data[0]);
        sl_app_log("SPO2 %d\r\n",sensor_data[1]);
        sl_app_log("Temp %d\r\n",sensor_data[2]);
        sensor_data[0] = 0;
